create_lambda_layer_dependencies:
	...
	python -m pip install -r source/trigger_sfn_lambda/requirements.txt -t source/trigger_sfn_lambda/__lambda_dependencies__  # machine that `cdk deploy` must have same Python version as deployed Lambdas, ie Python 3.9
	rm -rf source/trigger_sfn_lambda/__lambda_dependencies__/*.dist-info

	rm -rf source/vrf_fulfill_lambda/__lambda_dependencies__
	python -m pip install -r source/vrf_fulfill_lambda/requirements.txt -t source/vrf_fulfill_lambda/__lambda_dependencies__  # machine that `cdk deploy` must have same Python version as deployed Lambdas, ie Python 3.9
